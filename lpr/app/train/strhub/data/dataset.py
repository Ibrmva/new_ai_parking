...
def build_tree_dataset(root: Union[PurePath, str], *args, **kwargs):
    ...
    for lmdb_dir in glob.glob(str(root / '**/*_data.lmdb'), recursive=True):
        lmdb_path = Path(lmdb_dir)
        if lmdb_path.is_dir():
            ds_name = str(lmdb_path.relative_to(root))
            ds_root = str(lmdb_path.absolute())
            dataset = LmdbDataset(ds_root, *args, **kwargs)
            log.info(f'\tlmdb:\t{ds_name}\tnum samples: {len(dataset)}')
            datasets.append(dataset)
    if not datasets:
        raise ValueError("No LMDB datasets found in the specified root directory.")
